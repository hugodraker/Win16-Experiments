...
/* --- Utility Functions --- */
void DrawPDFDimensions(FILE* f, long* stream_len, Dimension* dims, int dimCount, Shape* shapes, int shapeCount, double pageScale, double offX, double offY, double minX, double minY, double pt_h) {
    int i;
    for (i = 0; i < dimCount; i++) {
        double A1x, A1y, A2x, A2y, D1x, D1y, D2x, D2y;
        double midX, midY, val; char buf[32];
        ...
        A1x = shapes[dims[i].s1].ptsX[dims[i].p1];
        A1y = shapes[dims[i].s1].ptsY[dims[i].p1];
        A2x = shapes[dims[i].s2].ptsX[dims[i].p2];
        A2y = shapes[dims[i].s2].ptsY[dims[i].p2];
        
        dx = A2x - A1x; dy = A2y - A1y;
        
        double pdfOffset = (dims[i].offset / 10.0) * pageScale;
        
        double pdfA1x = offX + (A1x - minX) * pageScale;
        double pdfA1y = pt_h - (offY + (A1y - minY) * pageScale);
        double pdfA2x = offX + (A2x - minX) * pageScale;
        double pdfA2y = pt_h - (offY + (A2y - minY) * pageScale);
        
        double pdfDx = pdfA2x - pdfA1x;
        double pdfDy = pdfA2y - pdfA1y;
        
        if (dims[i].mode == 0) { 
            ang = atan2(pdfDy, pdfDx);
            nx = -sin(ang); ny = cos(ang);
            D1x = pdfA1x + nx * pdfOffset; D1y = pdfA1y + ny * pdfOffset;
            D2x = pdfA2x + nx * pdfOffset; D2y = pdfA2y + ny * pdfOffset;
            val = sqrt(dx*dx + dy*dy);
        } else if (dims[i].mode == 1) { 
            D1x = pdfA1x; D1y = pdfA1y - pdfOffset;
            D2x = pdfA2x; D2y = pdfA1y - pdfOffset;
            val = fabs(dx);
        } else { 
            D1x = pdfA1x + pdfOffset; D1y = pdfA1y;
            D2x = pdfA1x + pdfOffset; D2y = pdfA2y;
            val = fabs(dy);
        }
        ...
        if (dimLen > 0) {
            ...
            pdf_out(f, stream_len, "%.2f %.2f m %.2f %.2f l %.2f %.2f l f\n",
                D1x, D1y,
                D1x + dirX*8 - dirY*2.5, D1y + dirY*8 + dirX*2.5,
                D1x + dirX*8 + dirY*2.5, D1y + dirY*8 - dirX*2.5);
            pdf_out(f, stream_len, "%.2f %.2f m %.2f %.2f l %.2f %.2f l f\n",
                D2x, D2y,
                D2x - dirX*8 - dirY*2.5, D2y - dirY*8 + dirX*2.5,
                D2x - dirX*8 + dirY*2.5, D2y - dirY*8 - dirX*2.5);
        }
    ...
    if (cur) {
        ...
        while (st < endBlock && ref->shapeCount < MAX_SHAPES) {
            ...
            if (next == pt) {
                ...
                char *nextPt = strstr(next + 1, "POINT ");
                s->type = (polyChk && (!nextPt || polyChk < nextPt)) ? 2 : 0;
    ...
    if (sh->type == 3) {
        ...
        if (rIdx != -1) {
            ...
            for (r = 0; r < 4; r++) {
                ...
                sh->ptsX[r + 1] = objCx + (dx * cosR - dy * sinR);
                sh->ptsY[r + 1] = objCy + (dx * sinR + dy * cosR);
            }
            sh->ptsX[5] = objCx; sh->ptsY[5] = objCy; 
...
void generate_pdf(const char* out_file, const char* in_file, float margH_pct, float margV_pct) {
    ...
    for (i=0; i<parsedCount; i++) {
        ...
        if (parsedIcons[i].dimCount > 0) {
            DrawPDFDimensions(f, &stream_len, parsedIcons[i].dims, parsedIcons[i].dimCount, parsedIcons[i].shapes, parsedIcons[i].shapeCount, scale, offX, offY, minX, minY, pt_h);
        }
