...
static void ParseIniEntry(const char* id, const char* val, IniShortcut* out) {
    ...
    out->minimized = (out->minimizedStr[0] == '1'); out->isFolder = (out->exe[0] == '\0'); out->hMenu = NULL;
}
...
static void SaveIniEntry(IniShortcut* item) {
    char val[1024]; sprintf(val, "%s|%s|%s|%s|%d|%s|%s", item->name, item->exe, item->params, item->icon, item->minimized, item->parentId, item->hotkey);
...
int CompareIni(const void* a, const void* b) {
    ...
    if (cmp == 0) {
        if (lstrcmp(sa->parentId, "0") == 0) {
            ...
            /* Programs (ID 1) is always the absolute top */
            ...
            /* Standard OS items (ID 2 to 7) remain in their ascending original order */
            return idA - idB;
        }
...
static void LoadIniShortcuts(void) {
    ...
    if (keys[0] == '\0') {
        WritePrivateProfileString("Shortcut", "00000001", "Programs||||0|0|", g_szIniPath);
        WritePrivateProfileString("Shortcut", "00000002", "Startup||||0|00000001|", g_szIniPath);
        WritePrivateProfileString("Shortcut", "00000003", "-||||0|0|", g_szIniPath);
        WritePrivateProfileString("Shortcut", "00000004", "Settings|||3|0|0|", g_szIniPath);
        ...
        UpdateIconInCache(1, NULL, NULL, TRUE); UpdateIconInCache(2, NULL, NULL, TRUE); UpdateIconInCache(4, NULL, NULL, TRUE);
        GetPrivateProfileString("Shortcut", NULL, "", keys, sizeof(keys), g_szIniPath);
    }
    pKey = keys;
    while (*pKey) {
...
/* ──────────────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────────── */
LRESULT CALLBACK ShortcutDlgProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    static HWND hName, hTarget, hParams, hIcon, hMinCheck, hFolderCheck, hParentFolder, hHotkey;
    switch(msg) {
        case WM_CREATE: {
            char name[MAX_PATH] = "", target[MAX_PATH] = "", params[MAX_PATH] = "", iconF[MAX_PATH] = "", parentId[16] = "0", hotkey[16] = ""; 
            int minimized = 0, isFolder = 0, i;
            ...
            if (g_EditShortcutId[0] != '\0') {
                for (i = 0; i < g_IniShortcutCount; i++) {
                    if (lstrcmp(g_IniShortcuts[i].id, g_EditShortcutId) == 0) {
                        ...
                        lstrcpy(parentId, g_IniShortcuts[i].parentId); lstrcpy(hotkey, g_IniShortcuts[i].hotkey);
                        minimized = g_IniShortcuts[i].minimized; isFolder = g_IniShortcuts[i].isFolder; break; } }
            ...
            CreateWindow("STATIC", "Hotkey (VK code):", WS_CHILD|WS_VISIBLE, 10, 160, 100, 20, hwnd, NULL, g_hInst, NULL);
            hHotkey = CreateWindowEx(0, "EDIT", hotkey, WS_CHILD|WS_VISIBLE|WS_BORDER|ES_AUTOHSCROLL, 110, 160, 80, 22, hwnd, NULL, g_hInst, NULL);

            hMinCheck = CreateWindow("BUTTON", "Start Minimized", WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX, 110, 190, 120, 20, hwnd, NULL, g_hInst, NULL);
            hFolderCheck = CreateWindow("BUTTON", "Is Folder", WS_CHILD|WS_VISIBLE|BS_AUTOCHECKBOX, 240, 190, 80, 20, hwnd, NULL, g_hInst, NULL);
            
            CreateWindow("BUTTON", "OK", WS_CHILD|WS_VISIBLE|BS_DEFPUSHBUTTON, 80, 230, 80, 24, hwnd, (HMENU)IDOK, g_hInst, NULL);
            CreateWindow("BUTTON", "Cancel", WS_CHILD|WS_VISIBLE, 180, 230, 80, 24, hwnd, (HMENU)IDCANCEL, g_hInst, NULL);
            ...
            if (minimized) SendMessage(hMinCheck, BM_SETCHECK, 1, 0); if (isFolder) SendMessage(hFolderCheck, BM_SETCHECK, 1, 0);
            SetFont(hwnd, NULL); SetFocus(hName); PositionDialogNearStart(hwnd); return 0;
        }
        case WM_COMMAND:
            if (wp == 101) { char path[MAX_PATH]; if (BrowseFile(hwnd, path, "Programs (*.exe;*.com)\0*.exe;*.com\0All Files (*.*)\0*.*\0")) { SetWindowText(hTarget, path); if (GetWindowTextLength(hIcon) == 0) SetWindowText(hIcon, path); } } 
                ...
                char name[MAX_PATH], target[MAX_PATH], params[MAX_PATH], iconF[MAX_PATH], hotkey[16], parentSel[128], *space;
                GetWindowText(hName, name, MAX_PATH); GetWindowText(hTarget, target, MAX_PATH); GetWindowText(hParams, params, MAX_PATH); GetWindowText(hIcon, iconF, MAX_PATH); GetWindowText(hHotkey, hotkey, 16);
                ...
                if (name[0] && (target[0] || SendMessage(hFolderCheck, BM_GETCHECK, 0, 0))) {
                    ...
                    lstrcpy(sh.name, name); lstrcpy(sh.exe, target); lstrcpy(sh.params, params); lstrcpy(sh.icon, iconF); lstrcpy(sh.hotkey, hotkey);
                    sh.minimized = SendMessage(hMinCheck, BM_GETCHECK, 0, 0) ? 1 : 0; sh.isFolder = SendMessage(hFolderCheck, BM_GETCHECK, 0, 0) ? 1 : 0;
                    if (sh.isFolder) { sh.exe[0] = '\0'; sh.params[0] = '\0'; }
...
LRESULT CALLBACK SearchBoxProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    if (msg == WM_KEYDOWN) {
        ...
        if (wp == VK_RETURN) { PostMessage(g_hTaskbar, WM_COMMAND, MAKELONG(g_hSearchList, LBN_DBLCLK), IDM_SEARCH_LIST); return 0; }
...
LRESULT CALLBACK SearchListProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp) {
    if (msg == WM_KEYDOWN && wp == VK_RETURN) { 
        SendMessage(g_hTaskbar, WM_COMMAND, MAKELONG(hwnd, LBN_DBLCLK), IDM_SEARCH_LIST); 
        return 0; 
    }
    if (msg == WM_LBUTTONUP) {
        RECT rc; POINT pt;
    ...
    switch (msg) {
        ...
        case WM_DRAWITEM: {
            ...
            if (lpdis->CtlType == ODT_MENU) {
                ...
                if (!item) return TRUE;
                if (lpdis->itemState & ODS_SELECTED) { hBrush = CreateSolidBrush(GetSysColor(COLOR_HIGHLIGHT)); FillRect(hdc, &rc, hBrush); SetTextColor(hdc, GetSysColor(COLOR_HIGHLIGHTTEXT)); } 
        ...
        case WM_RBUTTONUP: {
            ...
            g_MenuOpen = TRUE; TrackPopupMenu(hMenu, 0, pt.x, pt.y, 0, hwnd, NULL); g_MenuOpen = FALSE;
            DestroyMenu(hMenu);
            return 0;
        }
            ...
            if (id == ID_START_BUTTON) ShowStartMenu(g_hStartBtn);
                ...
                if (sel != CB_ERR) {
                    ...
                    if (idx >= 0 && idx < g_IniShortcutCount) {
                        ...
                        ShellExecute(hwnd, "open", sh->exe, sh->params, NULL, sh->minimized ? SW_SHOWMINIMIZED : SW_SHOWNORMAL);
                    }
                }
                ...
                if (idx < g_IniShortcutCount) { 
                    IniShortcut* sh = &g_IniShortcuts[idx]; 
                    if (lstrcmpi(sh->exe, "shutdown") == 0) ExitWindows(0, 0);
                    else if (lstrcmpi(sh->exe, "run") == 0) CreateCenteredDialog(g_hInst, hwnd, "RunDlgClass", "Run", 290, 160);
                    else ShellExecute(hwnd, "open", sh->exe, sh->params, NULL, sh->minimized ? SW_SHOWMINIMIZED : SW_SHOWNORMAL); 
                }
            }
                    ...
                    for (i = 0; i < g_IniShortcutCount; i++) {
                        if (g_IniShortcuts[i].hotkey[0] != '\0') {
                            ...
                            if (vk && (GetAsyncKeyState(vk) & 1)) {
                                ShellExecute(hwnd, "open", g_IniShortcuts[i].exe, g_IniShortcuts[i].params, NULL, g_IniShortcuts[i].minimized ? SW_SHOWMINIMIZED : SW_SHOWNORMAL);
                            }
                        }
                    }
                }
